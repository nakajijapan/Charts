...
open class CustomRadarChartRenderer: LineRadarRenderer
{
    ...
    @objc public init(chart: CustomRadarChartView, animator: Animator, viewPortHandler: ViewPortHandler)
    {
    ...
    open override func drawWebBackGround(context: CGContext) {
    ...
    )
    {
        ...
        if let cgImage = image?.cgImage, let size = image?.size {

            var flipVertical = CGAffineTransform(a: 1, b: 0, c: 0, d: -1, tx: 0, ty: 0)
            flipVertical.ty = size.height + point.y * 2

            context.concatenate(flipVertical)

            let movedPoint = CGPoint(x: point.x + offsetPoint.x, y: point.y + offsetPoint.y)
            context.draw(cgImage, in: CGRect(origin: movedPoint, size: size))
    ...
    private func createAccessibleElement(withDescription description: String,
                                         container: CustomRadarChartView,
